...
import me.badbones69.crazycrates.multisupport.holograms.DecentHologramsSupport;
import me.badbones69.crazycrates.multisupport.holograms.HologramsSupport;
...
public class CrazyManager {
    ...
    private Cache<UUID, Integer> rateLimiter = CacheBuilder.newBuilder()
    ...
    /**
     * Schematic locations for QuadCrates
     */
    ...
    public void givePrize(Player player, Prize prize) {
        if (prize != null) {
            ...
            for (String command : prize.getCommands()) { // /give %player% iron %random%:1-64
                ...
                if (Support.PLACEHOLDERAPI.isPluginLoaded()) command = PlaceholderAPI.setPlaceholders(player, command);
            ...
            for (String message : prize.getMessages()) {
                if (Support.PLACEHOLDERAPI.isPluginLoaded()) message = PlaceholderAPI.setPlaceholders(player, message);
    ...
    public boolean hasPhysicalKey(Player player, Crate crate, boolean checkHand) {
        ...
        if (checkHand) {
            ...
            if (ServerProtocol.isAtLeast(ServerProtocol.v1_9_R1)) items.add(player.getEquipment().getItemInOffHand());
     ...
     */
    public boolean takeKeys(int amount, Player player, Crate crate) {
        String uuid = player.getUniqueId().toString();
        String SQL = "CALL `takeKeys`(?, ?, ?);";
        try (Connection connection = plugin.getDatabase().getConnection(); PreparedStatement statement = connection.prepareStatement(SQL)) {
            ...
            statement.setString(2, crate.getName());
     ...
     */
    public void addKeys(int amount, Player player, Crate crate) {
        String uuid = player.getUniqueId().toString();
        String SQL = "CALL `giveKeys`(?, ?, ?);";
        try(Connection connection = plugin.getDatabase().getConnection(); PreparedStatement statement = connection.prepareStatement(SQL))
        {
            statement.setString(1, uuid);
            statement.setString(2, crate.getName());
    ...
    public void loadSchematics() {
        ...
        assert schems != null;
        for (String schematicName : schems) {
            if (schematicName.endsWith(".schematic")) crateSchematics.add(new CrateSchematic(schematicName.replace(".schematic", ""), new File(plugin.getDataFolder() + "/Schematics/" + schematicName)));
        }
    }
    ...
    public CrateSchematic getCrateSchematic(String name) {
        for (CrateSchematic schematic : crateSchematics) {
            if (schematic.getSchematicName().equalsIgnoreCase(name)) return schematic;
        }
